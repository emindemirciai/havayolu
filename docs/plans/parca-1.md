...
| — | Marka geçişi: ucus-takip → havayolu · havayolu.live (D-059) | `chore/havayolu-marka` | [k] kod bitti; push + PR bekliyor | — |
...
Önceki kayıtlardaki `ucus-takip` ve `ut-` adları o anda çalışan adlardır; değiştirilmedi.
...
- Eski `ucus-takip-dev` yerel altyapısı silindi; `pnpm dev:infra` → `havayolu-dev` 4 servis healthy. `pnpm test:integration` → 5/5. `pnpm compose:guard` → 8 servis ✓.
